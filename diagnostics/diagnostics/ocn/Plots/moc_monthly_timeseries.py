...
class MOCMonthly(OceanDiagnosticPlot):
    ...
    def __init__(self):
        super(MOCMonthly, self).__init__()
        self._expectedPlots = ['maxmoc']
        self._webPlotsDict = {'MOC':'maxmoc'}
        self._name = 'Monthly MOC Maximum'
        self._shortname = 'MOCMON'
        self._template_file = 'moc_monthly_timeseries.tmpl'
        self._ncl = list()

    def check_prerequisites(self, env):
        """list and check specific prequisites for this plot.
        """
        super(MOCMonthly, self).check_prerequisites(env)
        print("  Checking prerequisites for : {0}".format(self.__class__.__name__))

        # create link to MOC average file
        sourceFile = '{0}/{1}.pop.h.{2}-{3}.mocm.nc'.format(env['TAVGDIR'], env['CASE'], env['YEAR0'], env['YEAR1'])
        linkFile = '{0}/{1}.pop.h.MOC.{2}-01_cat_{3}-12.nc'.format(env['WORKDIR'], env['CASE'], env['YEAR0'], env['YEAR1'])
...
class MOCMonthly_timeseries(MOCMonthly):
    ...
    def __init__(self):
        super(MOCMonthly_timeseries, self).__init__()
        self._ncl = ['moc_monthly_timeseries.ncl']
