...
# Identical to maps_surface_diff_2D.py
...
for var in varlist:
        ...
        if 'tcfac' in variables[var]:
            ...
            if 'FLUX_IN' in var: # particle flux
                variables[var]['title_exp'] = 'Total %s at %.0f m = %.2f %s'%(
                    variables[var]['slabel'],depth,totalm,variables[var]['tunits'])
            else: # vertical integrals
                variables[var]['title_exp'] = 'Total %s = %.2f %s'%(
                    variables[var]['slabel'],totalm,variables[var]['tunits'])
    ...
    if os.path.isfile(infile_cntrl): # is file/variable available?
        ...
        if 'tcfac' in variables[var]:
            ...
            if 'FLUX_IN' in var: # particle flux
                variables[var]['title_cntrl'] = 'Exp-Con (Delta total = %.2f %s)'%(
        ...
        if var in vintlist: # vertically integrated
          CF  = map.contourf(x,y,expdata,variables[var]['cklev'],cmap=cmapmap,
                norm=colors.BoundaryNorm(variables[var]['cklev'],cmapmap.N),extend='both')
...
#       ax.set(title='Experiment - Control')
        ...
        if var in vintlist: # vertically integrated
          CF  = map.contourf(x,y,diffdata,variables[var]['dklev'],cmap=cmapdiff,
                norm=colors.BoundaryNorm(variables[var]['dklev'],cmapdiff.N),extend='both')
          CF.cmap.set_under('black')
          CF.cmap.set_over('black')
          CS  = map.contour(x,y,diffdata,CF.levels,colors = 'k',linewidths = 0.25)
          CB  = fig.colorbar(CF,ax=ax,drawedges=True,shrink=0.8,aspect=11,
                  ticks=variables[var]['dklev'][1:-1],format='%G')
          CB.set_label(variables[var]['kunits'])
        else:
