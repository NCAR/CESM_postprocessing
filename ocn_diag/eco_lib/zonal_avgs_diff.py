...
# Identical to zonal_avgs_diff.py except for the resource file.
        ...
        for r in range(nreg):
            if reg_lname[r] in region_list:
                ...
                if var in vintlist: # vertically integrated
                  CF  = ax.contourf(mon,lat,datam,variables[var]['cklev'],cmap=cmap,
                  ...
                  titlestrn = '%s %s from Experiment'%(reg_lname[r],variables[var]['klabel'])
                else:
                  ...
                  if 'FLUX_IN' in var:
                    titlestrn = '%s %s at %.2f m from Exp'%(reg_lname[r],variables[var]['slabel'],depth)
                  else:
